...
export default function SavingsGoalManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(isEditing ? "Savings goal updated successfully" : "Savings goal added successfully");
        resetForm();
        fetchSavingsGoals();
      } else {
        toast.error(isEditing ? "Failed to update savings goal" : "Failed to add savings goal");
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      <Card>
        <CardHeader>
          <CardTitle>{isEditing ? "Update Savings Goal" : "Create New Savings Goal"}</CardTitle>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Processing..." : isEditing ? "Update Goal" : "Create Goal"}
            </Button>
          </CardFooter>
        </form>
      </Card>
